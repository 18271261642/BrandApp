...
public class ActivityBindWrishBrand extends BaseMVPTitleActivity<BindBaseView, BindPresenter> implements BindBaseView {
    private RefreshRecyclerView refreshRecyclerView;
    private AdapterBindPageDeviceList adapterBindPageDeviceList;
    private boolean hasWatch;
    private boolean hasScale;
    private boolean hasBrand520;
    private boolean hasBrand;
    ArrayList<DeviceBean> list = new ArrayList<>();
    public static ActivityBindWrishBrand mActivityBind;
...
//                            }
    ...
    private void getDeviceList() {
        list.clear();
        list.add(new DeviceBean(JkConfiguration.DeviceType.BRAND_W307J, String.format(UIUtils.getString(R.string.detail_wristband), Constants.BRAND_W307J_FILTER), R.drawable.icon_w307j, 0));
        ...
        if (AppConfiguration.deviceMainBeanList != null) {
            for (int deviceType : AppConfiguration.deviceMainBeanList.keySet()) {
                ...
                } else if (deviceBean.currentType == JkConfiguration.DeviceType.BRAND_W311) {
                    hasBrand = true;
                    updateList(1, deviceBean);
                } else if (deviceBean.currentType == JkConfiguration.DeviceType.Brand_W520) {
                    hasBrand520 = true;
                    updateList(2, deviceBean);
                } else if (deviceBean.currentType == JkConfiguration.DeviceType.Brand_W814) {
        ...
        if (AppConfiguration.deviceMainBeanList != null && AppConfiguration.deviceMainBeanList.size() > 0) {
            ...
            if (Utils.isEmpty(mDeviceBean.deviceName)) {
                if (DeviceTypeUtil.isContainBrand() || DeviceTypeUtil.isContainW81() || DeviceTypeUtil.isContainWatch()) {
                    ...
                    ActivitySwitcher.goScanActivty(ActivityBindWrishBrand.this, mDeviceBean.currentType);
                }
    ...
    private void unBindDevice(DeviceBean deviceBean, boolean isderi) {
        isDerictUnBind = true;
        ...
        // }
        mActPresenter.unBind(deviceBean, isderi);
    }
    ...
    protected void initData() {
        ...
        titleBarView.setTitle(R.string.select_device_type_bracelet);
    ...
    public void onUnBindSuccess() {
        new Handler().postDelayed(new Runnable() {
            ...
            public void run() {
                ...
                EventBus.getDefault().post(new MessageEvent(MessageEvent.UNBIND_DEVICE_SUCCESS));
                GoActivityUtil.goActivityUnbindDevice(currentType, ActivityBindWrishBrand.this);
